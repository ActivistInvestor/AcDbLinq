...
/// An altered (hacked?) fork of ExpressionEqualityComparer
/// from Microsoft.EntityFrameworkCore.Query, located at:
/// 
///   https://github.com/dotnet/efcore/blob/main/src/EFCore/Query/ExpressionEqualityComparer.cs
///   
/// Changes:
/// 
/// The code that produces a hashcode for an expression
/// was modified to not use parameter names as ingredients
   ...
   public int GetHashCode(Expression obj)
   {
      ...
      unchecked
      {
         ...
         switch(obj)
         {
            ...
            // TT: Modified to exclude parameter name as
            //     an ingredient in hashcode generation:
            ...
            default:
               ...
               throw new NotSupportedException($"UnhandledExpressionNode: {obj.NodeType}");
         }
   ...
   private struct ExpressionComparer
   {
         ...
         return left switch
         {
            ...
            _ => left.NodeType == ExpressionType.Extension
                ...
                : throw new InvalidOperationException(
                   $"UnhandledExpressionNode: {left.NodeType}")
         };
      }
      ...
      private bool CompareLambda(LambdaExpression a, LambdaExpression b)
      {
         ...
         for(var i = 0; i < n; i++)
         {
            ...
            {
               throw new InvalidOperationException($"UnhandledExpressionNode: {p1.Name}");
            }
         }
...
#pragma warning restore IDE0066 // Convert switch statement to expression
         {
            ...
            default:
               throw new InvalidOperationException($"UnhandledExpressionNode: {a.BindingType}");
         }
      }
